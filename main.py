...
from spatial_features import (PaperDashboard, SkeletalTopology,
                               ObjectLearner, AutoPaperDetector,
                               GestureIntentEngine)          # PILLAR 1
...
class OmniDeskApp:
    def __init__(self, config_path='config.json'):
        ...
        # ============================================================
        # PILLAR 1 — inject GestureIntentEngine into HabitEngine
        # ============================================================
        self.habit_engine._gesture_intent = GestureIntentEngine(
            stability_threshold=self.config['system'].get('gesture_stability_threshold', 0.008),
            intent_frames=self.config['system'].get('gesture_intent_frames', 6))
        ...
        self.topology = SkeletalTopology(
            tolerance=self.config['system'].get('gesture_tolerance', 0.85),
            window=self.config['system'].get('gesture_dtw_window', 5))    # PILLAR 1
        self.obj_learner = ObjectLearner()
        self.auto_paper = AutoPaperDetector()                 # PILLAR 2 (adaptive)
        ...
        # PILLAR 4 — EWMA tick timer (fires every second on main thread)
        self._ewma_timer = None
    ...
    def _get_default_config(self):
        return {
            "system": {
                "camera_id": 0, "fps": 30, "ema_alpha": 0.3,
                "gesture_tolerance": 0.85, "frame_skip": 0,
                # PILLAR 1 additions
                "gesture_stability_threshold": 0.008,
                "gesture_intent_frames": 6,
                "gesture_dtw_window": 5,
            },
    ...
    def start(self, ui):
        ...
        self.ui.show_log_requested.connect(
            lambda: self.ui.show_log(
                self.habit_engine.activity_log,
                self.stats.get_today_stats().get("focus_score")))
        ...
        self.ui.pomodoro_finished.connect(
            lambda: self.habit_engine.speak("Pomodoro cycle complete. Take a break."))
        self.ui.config_updated.connect(self.handle_config_update)
        self.ui.request_input_signal.connect(self._handle_input_request)

        # PILLAR 4 — EWMA second-tick timer (runs on Qt main thread, safe)
        self._ewma_timer = QTimer()
        self._ewma_timer.timeout.connect(self._on_ewma_tick)
        self._ewma_timer.start(1000)   # every 1 second

        self.vision_thread = threading.Thread(target=self.run_vision, daemon=True)
        self.vision_thread.start()

    # PILLAR 4 — tick slot
    def _on_ewma_tick(self):
        if self.habit_engine.mode == "Focus" and self.stats:
            score, break_recommended = self.stats.ewma_tick()
            if break_recommended:
                # Only announce once per suggestion
                self.habit_engine.speak("Consider a short micro-break now.")
                self.habit_engine.log_event("💡 Micro-break recommended (high cognitive load)")
                self.stats.log_stat("micro_breaks_suggested", 1)
    ...
    def _get_input_threadsafe(self, title, label):
        ...
        try:
            return self._input_queue.get(timeout=30)
        except queue.Empty:
            return None
            ...
            try:
                ...
                if hands and hands.multi_hand_landmarks:
                    ...
                    if spatial_signature is None:
                        ...
                        return
                    user_action = self._get_input_threadsafe(
                        "AI Gesture Integration",
    ...
    def handle_config_update(self, new_config):
        ...
        # Re-inject updated GestureIntentEngine params
        if self.habit_engine._gesture_intent:
            self.habit_engine._gesture_intent.stability_threshold = \
                self.config['system'].get('gesture_stability_threshold', 0.008)
            self.habit_engine._gesture_intent.intent_frames = \
                self.config['system'].get('gesture_intent_frames', 6)
        self.update_voice_state()
    ...
    def update_voice_state(self):
        ...
        if enabled and not self.habit_engine._voice_enabled:
            ...
            self.habit_engine._voice_thread = threading.Thread(
                target=self.habit_engine.voice_command_listener,
                args=(self.handle_mode_change,), daemon=True)
    ...
    def start_object_learning(self):
        self.learning_object = True
        self.ui.update_status_signal.emit("Hold object in center...")
    ...
    def run_vision(self):
        ...
        while self.running:
            ...
            # 1. CORE TRACKING — Hands (every frame)
            results = self.vision.process(frame, features=['hands'])
            hands = results.get('hands')

            # 2. FACE MESH + GAZE (every 3 frames)
            if frame_count % 3 == 0:
                results.update(self.vision.process(frame, features=['face_mesh']))
                fm = results.get('face_mesh')

                # PILLAR 3 — compute gaze and feed to HabitEngine
                gx, gy, on_screen, blink_d = self.vision.gaze_estimator.estimate(fm)
                self.habit_engine.set_gaze(gx, gy, on_screen, blink_d)

                self.habit_engine.gaze_dimmer(fm)
                self.habit_engine.shush_trigger(hands, fm)

            # 3. POSTURE + PRIVACY (every 10 frames)
            if frame_count % 10 == 0:
            ...
            # 4. ADAPTIVE PAPER DASHBOARD (every 30 frames) — PILLAR 2
            if self.config['paper_dashboard'].get('auto_detect', True) and frame_count % 30 == 0:
                corners = self.auto_paper.detect(frame)   # shadow-aware smooth corners
                if corners:
                    if not self.db_connected:
                        self.habit_engine.speak("Dashboard connected")
                        self.db_connected = True
                    self.dashboard.set_corners(corners)
                else:
                    if self.db_connected:
                        self.db_connected = False
                        self.dashboard.set_corners([])

            # 5. OBJECT & SENSOR (every 5 frames)
            if frame_count % 5 == 0:
            ...
            # 6. AMBIENT LIGHT (every 15 frames)
            if frame_count % 15 == 0:
            ...
            if hands and hands.multi_hand_landmarks:
                ...
                for g in self.config.get('custom_gestures', []):
                    if self.topology.match(curr_sig, g['signature']) > self.topology.tolerance:
                        ...
                        subprocess.Popen(g['macro'], shell=True)
                        time.sleep(2)

                idx = hands.multi_hand_landmarks[0].landmark[8]
                macro = self.dashboard.check_tap(
                    (idx.x * self.vision.width, idx.y * self.vision.height))
                if macro:
            ...
            if self.ui.preview.isVisible():
                ...
                if hands and hands.multi_hand_landmarks:
                    for lm in hands.multi_hand_landmarks[0].landmark:
                        cv2.circle(p_f,
                                   (int(lm.x*self.vision.width), int(lm.y*self.vision.height)),
                                   3, (0, 255, 0), -1)
                if self.dashboard.corners:
                    for pt in self.dashboard.corners:
                        cv2.circle(p_f, tuple(map(int, pt)), 5, (255, 0, 0), -1)
